#!/usr/bin/env bash
# DuDuClaw OS boot health gate (H3c, 2026-08-23).
#
...
#     should roll back.
#   * The compositor / shell / kiosk session. The appliance's primary shape
#     is headless (duduclaw-kiosk.service is a clean skip with no display
#     attached), so gating on the display would leave every headless box
#     permanently unblessable — i.e. it would roll back healthy machines.
...
# body. Measured in the appliance VM (2026-08-23): the reply to this probe is
#   {"audit_id":"…","ok":false,"error":{"kind":"unauthorized",…}}
# — root is not the configured peer uid (1000, the gateway), and that refusal
# is *positive proof the daemon is alive, parsing and answering*, which is
# exactly what the gate needs to know. Cheaper and more honest than inferring
# liveness from an error code.
#
